...
- **服务端驱动内容**  
  所有卡片数据（类型、文本、图片 URL、排版方式等）均由（模拟）服务端下发，前端通过 `@Composable` 动态渲染。
  
- **下拉刷新 & 无限加载**  
  使用 `pullRefresh` + `LazyColumn` 实现流畅的下拉刷新；通过监听滚动位置触发分页加载。

- **删卡操作**  
  长按任意卡片弹出 `DelectDialog` 确认删除，确认后从列表中移除并同步至本地缓存。
...
- **混排布局控制**  
  每张卡片可独立指定排版：`SINGLE_COLUMN` 或 `DOUBLE_COLUMN`，由服务器发送数据决定。
...
- 加载初始化页面时显示骨架图，提高用户体验
- 延迟加载非必要组件，提高初始页面加载速度
- Job控制防抖动，避免重复的UI操作
- 卡片预渲染，UI 提前绘制为静态 Bitmap 并缓存
